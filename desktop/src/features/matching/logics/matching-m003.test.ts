...
function user(name: string, xId: string, casts: string[]): UserBean {
  return { name, x_id: xId, casts, raw_extra: [] };
}

function cast(name: string, ngAccountId?: string): CastBean {
  return {
    name,
    is_present: true,
...
describe('runMultipleMatching', () => {
  ...
  it('応募者または出勤キャストがない場合は空の結果を返す', () => {
    ...
    expect(runMultipleMatching([user('Alice', '@alice', ['Cast A'])], [{ name: 'Resting', is_present: false }], {
      usersPerTable: 1,
      castsPerRotation: 1,
      rotationCount: 1,
    }, 'accountId', 'exclude')).toEqual({ userMap: new Map() });
