...
const NOTICE_SECTION_TITLE = '注意事項';
...
const FEATURE_SAMPLE_META: Record<FeatureId, FeatureSampleMeta> = {
  ...
  lottery: {
    ...
    points: [
      ...
      { number: 4, title: '設定ステータス', description: '条件設定の一部として、ERROR、WARN、INFOで妥当性と合計席数を表示します。', x: 73, y: 36 },
...
const FEATURE_CONTENT: Record<FeatureId, React.ReactNode> = {
  ...
  'applicant-data': (
    <div>
      ...
      <Section title="主な機能">
        <FeatureList items={[
          '応募者一覧の表示（ユーザー名・X ID・希望キャスト。順位なしは希望キャスト欄内で最大希望数に合わせて整列表示）',
    ...
    <div>
      ...
      <Section title="列マッピング">
        <FeatureList items={[
          ...
          '希望キャスト：順位ありは1列ずつ指定、順位なしはカンマ区切り1列を選択。順位なしは希望キャスト欄内で整列表示し、全希望キャストを同じ50点で評価',
        ]} />
  ...
  'lottery': (
    <div>
      ...
      <ScreenSample title="抽選設定">
        <div style={{ display: 'flex', gap: 16 }}>
          ...
          <div style={{ width: 130, padding: '10px 12px', background: 'var(--guide-accent-lottery-bg)', border: '1px solid var(--guide-accent-lottery-border)', borderRadius: 6 }}>
            <div style={{ fontSize: 10, color: 'var(--guide-accent-lottery)', fontWeight: 700, marginBottom: 4 }}>INFO</div>
      ...
      <Section title="設定ステータス">
        <p>条件設定内でエラーや警告を自動チェックします。キャスト出席状況・合計席数・合計当選者数のバランスが合わない場合は警告が表示されます。INFOには合計席数と合計当選者数を表示します。</p>
      </Section>
                ...
                <div style={{ padding: '14px 20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  <ScreenSample title="抽選設定">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                      ...
                      <div style={{ marginTop: 4, padding: '5px', background: 'var(--guide-accent-lottery-bg)', border: '1px solid var(--guide-accent-lottery-border)', borderRadius: 5, fontSize: 10, color: 'var(--guide-accent-lottery)', fontWeight: 700 }}>INFO — 合計席数: 12席</div>
