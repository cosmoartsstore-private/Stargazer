...
describe('buildXProfileUrl', () => {
  it.each([
    ...
    ['@Sample_User', 'https://x.com/Sample_User'],
    ['https://twitter.com/Sample_User/status/1', 'https://x.com/Sample_User'],
...
describe('cast NG entries', () => {
  it('入力をトリムし、大文字小文字を保った@ID形式へ変換する', () => {
    expect(createCastNgEntry({
      username: '  Alice  ',
      accountId: 'https://x.com/MixedCase',
      notes: '  理由メモ  ',
    })).toEqual({
      username: 'Alice',
      accountId: '@MixedCase',
  ...
  it('保存表記のusernameとaccountIdがともに一致する場合だけ重複とする', () => {
    ...
    expect(isDuplicateCastNgEntry(entries, { username: 'alice', accountId: '@Sample' })).toBe(false);
    expect(isDuplicateCastNgEntry(entries, { username: 'Alice', accountId: '@sample' })).toBe(false);
...
describe('caution user conversion', () => {
  ...
  it('手動登録の名前が空なら整形後のX IDを表示名に使う', () => {
    ...
    expect(createManualCautionUser({ username: 'Alice', accountId: '', reason: '', notes: '' }, 'registered-at')).toBeNull();
  });
