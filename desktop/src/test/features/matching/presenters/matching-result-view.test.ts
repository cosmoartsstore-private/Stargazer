...
const castA: CastBean = { id: 1, name: 'Cast A', is_present: true };
const castB: CastBean = { id: 2, name: 'Cast B', is_present: true };
const inactiveCast: CastBean = { id: 3, name: 'Inactive', is_present: false };
...
describe('matching result presenters', () => {
  ...
  it('キャスト別行は出勤キャスト順を保ち、割り当てがある外部キャストも含める', () => {
    const externalCast: CastBean = { id: 4, name: 'External', is_present: true };
  ...
  it('同名でもIDが異なるキャストは別の結果行へ集計する', () => {
    const firstCast: CastBean = { id: 1, name: '同名キャスト', is_present: true };
    const secondCast: CastBean = { id: 2, name: '同名キャスト', is_present: true };
    const firstMatch: MatchedCast = { cast: firstCast, rank: 1 };
    const secondMatch: MatchedCast = { cast: secondCast, rank: 0 };

    const rows = buildCastResultRows([
      { user: alice, matches: [firstMatch] },
      { user: bob, matches: [secondMatch] },
    ], [firstCast, secondCast]);

    expect(rows.map((row) => row.cast.id)).toEqual([1, 2]);
    expect(rows[0].assignments).toEqual([{ user: alice, match: firstMatch }]);
    expect(rows[1].assignments).toEqual([{ user: bob, match: secondMatch }]);
  });

  it('希望表示を rank と score から分類する', () => {
    expect(getMatchPreference({ ...aliceMatch, rank: 1 })).toEqual({ label: '第1希望', tone: 'First' });
    expect(getMatchPreference({ ...aliceMatch, rank: 2 })).toEqual({ label: '第2希望', tone: 'Second' });
    expect(getMatchPreference({ ...aliceMatch, rank: 3 })).toEqual({ label: '第3希望', tone: 'Third' });
  ...
  it('キャスト別結果の列キーと列ラベルを返す', () => {
    ...
    expect(getCastResultColumnLabel(0)).toBe('第1ラウンド');
  });
  ...
  it('失敗理由ごとにユーザー向け文言を返す', () => {
    ...
    expect(formatFailureMessage('ng-conflict')).toContain('NG条件');
    expect(formatFailureMessage(undefined)).toContain('NG条件');
  });
});
