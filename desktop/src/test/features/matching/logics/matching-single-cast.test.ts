...
describe('runSingleCastMatching', () => {
  it('出勤キャストだけを使い、指定テーブル数まで空席を補う', () => {
    ...
    const result = runSingleCastMatching(users, casts, 3, 1, true);
  ...
  it('応募者数またはローテーション数が出勤キャスト数を超える場合は容量不足にする', () => {
    expect(runSingleCastMatching(users, [casts[0]], 2, 1, true)).toMatchObject({
      ngConflict: true,
      failureReason: 'insufficient-capacity',
    });
    expect(runSingleCastMatching([users[0]], casts, 1, 3, true)).toMatchObject({
      ngConflict: true,
      failureReason: 'insufficient-capacity',
    });
  });
  ...
  it('応募者または出勤キャストがない場合は空の結果を返す', () => {
    expect(runSingleCastMatching([], casts, 1, 1, true)).toEqual({
      userMap: new Map(),
    });
    expect(runSingleCastMatching(users, [{ id: 3, name: 'Resting', is_present: false }], 1, 1, true)).toEqual({
      userMap: new Map(),
    });
  });
  ...
  it('登録順モードではランダム化しない', () => {
    ...
    runSingleCastMatching(users, casts, 2, 1, false);
