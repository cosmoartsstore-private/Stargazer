...
import { buildTsvContent, downloadTsv } from './downloadCsv';
...
describe('buildTsvContent', () => {
  it('セルをタブで連結し、行を CRLF で連結する', () => {
    expect(buildTsvContent([
      ['name', 'x_id'],
      ['応募者A', 'x_id_a'],
      ['応募者B', 'x_id_b'],
    ])).toBe('name\tx_id\r\n応募者A\tx_id_a\r\n応募者B\tx_id_b');
  ...
  it('セル内のタブと改行を半角スペースに置換し、前後空白を取り除く', () => {
    expect(buildTsvContent([['  応募者\tA  ', 'line1\r\nline2\nline3']])).toBe(
      '応募者 A\tline1 line2 line3',
    );
  });

  it('数値セルを文字列化する', () => {
    expect(buildTsvContent([['count', 0], ['score', 42]])).toBe('count\t0\r\nscore\t42');
  });

  it('null と undefined のセルを空セルとして扱う', () => {
    expect(buildTsvContent([['name', null, undefined, 'memo']])).toBe('name\t\t\tmemo');
  });
