...
export function detectColumnMapping(headers: readonly string[]): ColumnMapping {
  ...
  for (let index = 0; index < headers.length; index += 1) {
    ...
    if (!header) continue;
    if (mapping.name < 0 && /名前|お名前|ユーザー名|name/.test(header)) { mapping.name = index; continue; }
    if (mapping.x_id < 0 && /x[\s_]?id|twitter|xid|アカウントid/.test(header)) { mapping.x_id = index; continue; }
    ...
    if (/カンマ区切り/.test(header) || /^希望キャスト$/.test(header) || /^希望$/.test(header)) {
      mapping.cast1 = index;
