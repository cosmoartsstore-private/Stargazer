...
- **Cast Management** — キャスト情報の登録（別名・写真・プロフィール・連絡先付き）
- **Data Import** — TSV からの応募者データ取り込み（空・重複 X ID の警告と手動削除、キャスト希望対応）
...
- 取込セッション DB: `meta`, `applicants`, `applicant_casts`, `applicant_extra`, `session_workflow_state`, `lottery_results`, `lottery_saved_runs`, `lottery_saved_run_results`

旧DBとの互換性を保つため、イベント共有DBの `event_cast_present` と `header_templates` も削除せず保持します。現行UIの出席状態は `casts.is_attend` を使用し、ヘッダーテンプレートは通常画面からは操作しません。

Stargazer 自身はルート DB を持ちません。`apps` は外部アプリである StellaRecord の DB へ連携情報を登録するときだけ使用します。

応募者、確定当選者、抽選・マッチング条件、現行抽選結果は取込セッション DB を正とします。明示的に保存した抽選を開くと、当選者に加えて保存時の抽選・マッチング方式と抽選人数を復元します。保存時の表示名は日時と当選人数から自動生成します。

空または正規化後に重複する X ID が存在する場合は該当行を警告し、利用者が取込画面または応募データ一覧から手動削除するまで抽選とマッチングを利用できません。旧DBに残る不正行も同じ規則で扱い、自動削除や暗黙の除外は行いません。

要注意ユーザーと候補閾値はイベント共有 DB を正とします。自動候補はキャスト NG から都度算出し、明示的に追加したユーザーだけを固定登録します。候補集計と登録時の重複判定は、保存されているX IDの表記が完全に一致する場合だけ同じIDとして扱います。固定登録と応募者の照合時は大文字小文字と先頭の `@` を区別しませんが、応募者名と登録名が両方ある場合は名前も一致した人を要注意として扱います。

localStorage へ保存するのはテーマ、探索モード、最後に開いたイベント・セッションなどの端末設定だけです。旧版の業務データ用localStorageキーは、現行の抽選・マッチング状態の復元には使用しません。旧形式DBと旧localStorageの移行方法は未決定です。

応募者・キャスト・出席・抽選結果の複数 SQL を伴う保存処理は、Rust 側 command で単一 SQLite transaction として実行します。

希望キャストはイベント共有DBの安定IDで識別し、取込セッションDBにはIDと取込時名称を保存します。取込時はキャストの正式名と登録済みの別名をIDへ解決します。同じ名前が複数のキャストに該当する場合は自動で決めません。IDの由来を確認できない旧形式データも、現在の名簿から名称だけで結び直しません。キャストを削除して同名で再登録しても別キャストとして扱うためです。旧形式データを再取込すると、その時点の名簿からIDを確定します。

削除済み・未解決の希望キャストがある場合は、希望どおりに判定できない可能性を画面で警告します。抽選とマッチングは継続でき、M000ではキャストを参照しないため警告も表示しません。マッチング結果自体はDBへ保存しないため、再起動後には復元されません。詳しい状態の寿命は [Architecture](docs/ARCHITECTURE.md#matching) を参照してください。

現行インストーラーの通常アンインストールでは `Data/archive` と通常のイベントディレクトリを残しますが、`Data/db`、`Data/logs`、旧WebView profileは削除します。旧データやイベント名 `db`・`logs` のデータが必要な場合は、アンインストール前に `Data` フォルダーを退避してください。
